...
class ActorViewModel extends ViewModel {
    ...
    public function knowForMovies() {
        ...
        return collect($castMovies)->where('media_type', 'movie')->take(5)->map(function ($movie) {
    ...
    public function credits() {
        ...
        return collect($castMovies)->map(function ($movie) {
            ...
            return collect($movie)->merge([
                ...
                'releaseYear' => isset($releaseDate) ? Carbon::parse($releaseDate)->format('Y') : 'Future',
                'title' => $title,
                'character' => isset($movie['character']) ? $movie['character'] : '',
                'linkToPage' => $movie['media_type'] === 'movie' ? route('show_movie', $movie['id']) : route('show_tv', $movie['id'])
            ])->only(['releaseDate', 'releaseYear', 'title', 'character', 'linkToPage']);
